...
const metricOptions = {
  temperature: { buttonLabel: 'Temperature', title: 'Temperature History', datasetLabel: 'CPU Temperature', field: 'temperature', unit: '°C', axisTitle: 'Temperature (°C)', borderColor: '#f97316', backgroundColor: '#fed7aa', suggestedMin: 30, suggestedMax: 80 },
  cpuUsage: { buttonLabel: 'CPU Usage', title: 'CPU Usage History', datasetLabel: 'CPU Usage', field: 'cpuUsage', unit: '%', axisTitle: 'CPU Usage (%)', borderColor: '#2563eb', backgroundColor: '#bfdbfe', suggestedMin: 0, suggestedMax: 100 },
  memoryUsage: { buttonLabel: 'Memory Usage', title: 'Memory Usage History', datasetLabel: 'Memory Usage', field: 'memoryUsage', unit: '%', axisTitle: 'Memory Usage (%)', borderColor: '#7c3aed', backgroundColor: '#ddd6fe', suggestedMin: 0, suggestedMax: 100 },
}

const rangeOptions = {
  '1h': { label: '1 hour', shortLabel: '1H', durationMs: 60 * 60 * 1000, bucketSeconds: 60 },
  '6h': { label: '6 hours', shortLabel: '6H', durationMs: 6 * 60 * 60 * 1000, bucketSeconds: 300 },
  '24h': { label: '24 hours', shortLabel: '24H', durationMs: 24 * 60 * 60 * 1000, bucketSeconds: 900 },
  '7d': { label: '7 days', shortLabel: '7D', durationMs: 7 * 24 * 60 * 60 * 1000, bucketSeconds: 7200 },
  '14d': { label: '14 days', shortLabel: '14D', durationMs: 14 * 24 * 60 * 60 * 1000, bucketSeconds: 14400 },
}
...
const valueLabelsPlugin = {
  ...
  afterDatasetsDraw(chart, _args, options) {
    ...
    chart.data.datasets.forEach((dataset, datasetIndex) => {
      ...
      const labelStep = Math.max(1, Math.ceil(points.length / (chart.width < 640 ? 6 : 12)))
      points.forEach((point, dataIndex) => {
        if (dataIndex % labelStep !== 0 && dataIndex !== points.length - 1) return
...
ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Tooltip, Legend, valueLabelsPlugin, thresholdLinePlugin)
...
async function fetchSummary() {
  ...
  try {
    const response = await api.get('/api/sensors/history/summary', {
      params: { from: toKoreanLocalIso(rangeStart.value), to: toKoreanLocalIso(rangeEnd.value), bucketSeconds: selectedRange.value.bucketSeconds },
    })
...
function movePrevious() {
  const nextRange = moveRangeEnd(
    rangeEnd.value,
    selectedRange.value.durationMs,
    'previous',
    Date.now(),
  )
...
watch(() => sensorStore.sensorEventVersion, () => {
...
const pointColors = computed(() => chartSensors.value.map((sensor) => (
  selectedMetricKey.value === 'temperature' && sensor.temperature >= TEMPERATURE_WARNING_CELSIUS ? '#dc2626' : selectedMetric.value.borderColor
)))
const chartData = computed(() => ({
  ...
  datasets: [{
    ...
    pointBorderColor: '#ffffff', pointBorderWidth: 2, pointRadius: 3, pointHoverRadius: 6,
...
const chartOptions = computed(() => ({
  ...
  layout: { padding: { top: 22 } }, interaction: { intersect: false, mode: 'index' },
  plugins: {
    legend: { display: false },
    tooltip: { callbacks: { label: (context) => `${selectedMetric.value.datasetLabel}: ${context.parsed.y.toFixed(1)}${selectedMetric.value.unit}` } },
    valueLabels: { unit: selectedMetric.value.unit },
    thresholdLine: { display: selectedMetricKey.value === 'temperature', value: TEMPERATURE_WARNING_CELSIUS, color: '#dc2626' },
  },
  scales: {
    x: { grid: { display: false }, ticks: { maxRotation: 40, minRotation: 0, autoSkip: true, maxTicksLimit: 10 } },
    y: { suggestedMin: selectedMetric.value.suggestedMin, suggestedMax: selectedMetric.value.suggestedMax, grace: '12%', title: { display: true, text: selectedMetric.value.axisTitle } },
  },
}))
function formatStatistic(value) { return value == null ? '--' : `${value.toFixed(1)}${selectedMetric.value.unit}` }

onMounted(() => { void fetchSummary() })
onUnmounted(() => { if (liveRefreshTimer != null) window.clearTimeout(liveRefreshTimer) })
</script>
  ...
  <section class="chart-panel">
    ...
    <div class="chart-controls">
      <div class="metric-tabs" role="group" aria-label="Chart metric">
        <button v-for="(metric, key) in metricOptions" :key="key" type="button" class="control-tab" :class="{ 'control-tab--active': selectedMetricKey === key }" :aria-pressed="selectedMetricKey === key" @click="selectedMetricKey = key">{{ metric.buttonLabel }}</button>
      </div>
      <div class="range-tabs" role="group" aria-label="Chart time range">
        <button v-for="(range, key) in rangeOptions" :key="key" type="button" class="control-tab" :class="{ 'control-tab--active': selectedRangeKey === key }" :aria-pressed="selectedRangeKey === key" :title="`Show ${range.label}`" @click="selectRange(key)">{{ range.shortLabel }}</button>
      </div>
    </div>
    <p v-if="errorMessage" class="chart-error">{{ errorMessage }}</p>
    <p v-else-if="isLoading" class="chart-loading">그래프 데이터를 불러오는 중입니다.</p>
    <div class="statistics-grid">
      <article><span>Average</span><strong>{{ formatStatistic(rangeStatistics?.average) }}</strong></article>
      <article><span>Minimum</span><strong>{{ formatStatistic(rangeStatistics?.minimum) }}</strong></article>
      <article :class="{ 'statistic--warning': hasTemperatureWarning }"><span>Maximum</span><strong>{{ formatStatistic(rangeStatistics?.maximum) }}</strong></article>
    </div>
    <div v-if="chartSensors.length > 0" class="chart-wrapper"><Line :data="chartData" :options="chartOptions" /></div>
    <div v-else-if="!isLoading" class="chart-empty">No sensor data exists in the selected time range.</div>
  </section>
</template>
...
.statistics-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 18px 0; }.statistics-grid article { display: grid; gap: 4px; padding: 12px; border-radius: 10px; background: #f8fafc; }.statistics-grid span { color: #64748b; font-size: 12px; }.statistics-grid strong { color: #0f172a; font-size: 17px; }.statistic--warning strong { color: #dc2626; }.chart-wrapper { height: 360px; }.chart-empty, .chart-loading, .chart-error { padding: 32px; text-align: center; }.chart-error { color: #b91c1c !important; }
@media (max-width: 720px) { .chart-panel__header, .chart-controls { flex-direction: column; }.chart-actions { width: 100%; }.statistics-grid { grid-template-columns: 1fr; }.chart-wrapper { height: 300px; } }
</style>
